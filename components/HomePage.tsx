...
import { HOME_SERVICES, CORPORATE_SERVICES, SERVICE_CATEGORY_DETAILS, LOCATION_SPECIFIC_TOUR_DETAILS, LOCATIONS, TOUR_SERVICES } from '../constants';
...
const HomePage: React.FC<HomePageProps> = ({ 
  ...
  const renderTourCategories = () => {
    const availableTourCategories = TOUR_SERVICES;

    if (availableTourCategories.length === 0) {
        return (
            <div className="text-center py-10 animate-fade-in">
                <p className="text-xl text-gray-600">No tour packages are available at the moment.</p>
                <p className="text-gray-500 mt-2">Check back soon for exciting new adventures!</p>
            </div>
        );
    }
    ...
    return (
        <div className="animate-fade-in grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {availableTourCategories.map(category => {
                const details = (selectedLocation && LOCATION_SPECIFIC_TOUR_DETAILS[selectedLocation]?.[category]) || SERVICE_CATEGORY_DETAILS[category]!;
                if (!details) return null;
                ...
                return (
                    ...
                    >
                        {(selectedLocation && LOCATION_SPECIFIC_TOUR_DETAILS[selectedLocation]?.[category]) && (
